...
private size_t peek_operations;
private size_t max_allowable_peek_operations;
...
/*
	...
	-----------
*/
private char peek_next(size_t distance = 0)
{
...
/*
	...
	Digit:
		...
		9
*/
private size_t number(size_t max_digits = size_t.max)
{
    size_t answer = 0;
...
/*
	...
	LName:
		Number Name
*/
private const(char)[] LName(size_t max_digits)
{
    const(char)[] name;
...
/*
	...
	HexDigit:
		...
		F
*/
private const(char)[] HexDigits()
{
    const(char)[] answer;

    while (true) switch (peek_next())
    {
    case '0': .. case '9':
    case 'A': .. case 'Z':
        answer ~= get_next();
...
/*
	...
	--------
*/
private size_t nybble()
{
    auto character = get_next();
    return character >= '0'
        && character <= '9' ? character - '0' : character >= 'A'
        && character <= 'F' ? character - 'A' + 10 : character >= 'a'
        && character <= 'f' ? character - 'a' + 10 : 0;
}
...
/*
	...
	-----------------
*/
private size_t byte_hex_number()
{
    switch (peek_next())
    {
    case '0': .. case '9': // a negative number (That's what the N stands for)
...
/*
	...
	--------
*/
private const(char)[] to_hex(ulong number, size_t digits)
{
...
/*
	...
	---------------
*/
private const(char)[] format_value(char sign, ulong value, char as)
{
    switch (as)
    {
...
/*
	...
	CharWidth:
		...
		d
	*/
private const(char)[] Value(const(char)[] name = null, char parent_type = '\0')
{
    const(char)[] answer;
    size_t length;
...
/*
	...
	TemplateArgX:
		...
		S LName
*/
private const(char)[] TemplateArgs()
{
    size_t count = 0;
    const(char)[] answer;
...
*/
private const(char)[] QualifiedName()
{
    const(char)[] name;
    const(char)[] part;
    size_t parts = 0;
...
/*
	...
	Wild:
		Ng
*/
private const(char)[] TypeModifiers()
{
...
/*
	...
	CallConvention:
		 ...
		 R       // C++
*/
private const(char)[] CallConvention()
{
    switch (peek_next())
    {
    case 'F':
...
/*
	...
	----------------
*/
private bool is_CallConvention()
{
    switch (peek_next())
    {
    case 'F':
...
*/
private const(char)[] TypeFunction(const(char)[] name, bool is_deligate = false, bool in_struct = false)
{
    ...
    if (!in_struct)
        return convention ~ func_attrs ~ the_type ~ (the_type.length == 0 ? "" : " ") ~ name ~ (
            is_deligate ? "delegate" : "") ~ "(" ~ params ~ ")";
    else
        return name ~ "(" ~ params ~ ")";
}
...
[*]		TypeTuple				/// THESE DON'T APPEAR TO EXIST SO THEY ARE UNIMPLEMENTED
	...
	Internal:
		Z
*/
private const(char)[] TypeX(const(char)[] name = "", bool allow_deligates = false, bool in_struct = false)
{
...
/*
	...
	Type:
		...
		TypeX
*/
private const(char)[] Type(const(char)[] name, bool allow_deligates = false,
    bool is_function = false, bool in_struct = false)
{
    const(char)[] answer;
...
/*
	...
	--------
*/
private const(char)[] decode(bool is_template = false)
{
    const(char)[] name;
